...
app.layout = html.Div(
    [
        ...
        dbc.Modal(
            [dbc.ModalHeader(dbc.ModalTitle("Header"), close_button=True), "test"],
            id="modal",
        ),
        html.Div(id="output"),
        dag.AgGrid(
        ...
        html.Div(
            id="testing",
            style={"position": "absolute", "top": "101vh"},
            children="testing",
        ),
    ]
)
...
@app.callback(Output("modal", "is_open"), Input("test_click", "n_clicks"))
def show(n):
    if n:
        return True


@app.callback(Output("output", "children"), Input("testing_type", "value"))
def typing(v):
    return v
